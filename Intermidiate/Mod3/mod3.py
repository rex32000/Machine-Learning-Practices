...
#initializing weights of nn for otimal learning
def weights_init(m):#m=object representing nn
...
class ActorCritic(torch.nn.Module):
    ...
    def __init__(self, num_inputs, action_space):
        ...
        self.actor_linear.bias.data.fill_(0)#just to be sure

        self.critic_linear.weight.data = normalized_columns_initializer(self.critic_linear.weight.data, 1)
        self.critic_linear.bias.data.fill_(0)  # just to be sure

        self.lstm.bias_ih.data.fill_(0)#initializing bis to zero
        self.lstm.bias_hh.data.fill_(0)

        self.train()#putting module in train mode
