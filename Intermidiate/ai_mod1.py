...
#Implementation experience replay
...
class Dqn():
    def __init__(self, input_size, nb_action, gamma):
        self.gamma =  gamma
        self.reward_window = []
        self.model = Network(self, input_size, nb_action)
    ...
    def learn(self, batch_state, batch_next_state, batch_reward, batch_action):
        outputs = self.model(batch_state).gather(1, batch_action.unsqueeze(1)).queeze(1)
        next_outputs = self.model(batch_next_state).detach().max(1)[0]
        targets = self.gamma*next_outputs + batch_reward
        td_loss = F.soomth_l1_loss(outputs, targets)
