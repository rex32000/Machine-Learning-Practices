...
class MDNRNN(object):
    ...
    def __init__(self, hps, reuse=False,gpu_node=False):
        self.hps = hps
...
#making method that create MDNRNN architecture
    def build_model(self, hps):
        #building rnn
        self.num_mixtures = hps.num_mixture
        ...
        if use_recurrent_dropout:
            cell = cell_fn(hps.rnn_size, layer_norm = use_layer_norm, droput_keep_prob = hps.recurrent_dropout_prob)
        else:
            cell = cell_fn(hps.rnn_size, layer_norm=use_layer_norm, droput_keep_prob=hps.recurrent_dropout_prob)
        if use_input_dropout:
            cell = tf.nn.rnn.cell.DropoutWrapper(cell, input_keep_prob = self.hps.input_dropout_prob)
        if use_output_dropout:
            cell = tf.nn.rnn.cell.DropoutWrapper(cell,  input_keep_prob = self.hps.output_dropout_prob)
        self.cell = cell
        self.sequence_lengths = LENGTH
        self.input_x = tf.placeholder(dtype = tf.float32, shape = [self.hps.batch_size, self.hps.mx_seq_len, INWIDTH])
        self.output_x = tf.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.mx_seq_len, OUTWIDTH])
        ...
        def get_lossfunc(logmix, mean, logstd, y):
            ...
            v = tf.math.reduce_mean(v, 1, keepdims = True)
        ...
        if self.hps.is_training == 1:
            ...
            capped_gvs = [(tf.clip_by_value(grad, -self.hps.grap_clip, self.hps.grap_clip), var) for grad, var in gvs]
